...
| NVIDIA execution | Keep cudarc 0.18.2 plus narrow dynamic cuSPARSE/cuBLAS bindings as the implementation baseline | `cuda-12000` is the tested binding ABI baseline, not “current CUDA”. Exact Q1/TPFA CG/Jacobi and fixed-reference FSI MINRES/identity tuples are implemented; their privacy-safe selected-device observations must be recollected from public source before they return to verified status. Neither implies a general CUDA, reproducible-device, hardware-support-matrix, or scale claim. A cudarc 0.19 upgrade requires fresh API, ABI and physical-device evidence; follow [cuSPARSE](https://docs.nvidia.com/cuda/cusparse/index.html) and [cuBLAS](https://docs.nvidia.com/cuda/cublas/) reproducibility rules |
...
| Gmsh parsing | Retain exact `mshio` 0.4.2 behind Eqiora preflight | Trust only the Eqiora-preflighted ASCII/binary MSH 4.1 subset and reconstruct through owned mesh constructors; upstream parser acceptance alone never widens the supported grammar. See [mshio](https://github.com/w1th0utnam3/mshio) |
...
- External parsers may silently ignore unsupported sections; Eqiora preflight
  must reject semantics it cannot reconstruct.
