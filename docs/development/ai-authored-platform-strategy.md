...
| Predicate | Threshold |
| --- | --- |
| File size | Production `.rs` under `crates/` must remain ≤ 1,000 physical lines and tests ≤ 2,000; existing line-count excess is frozen at its current value. In addition, every `.rs` discovered under `crates/` and `tools/` has absolute no-debt limits of 131,072 raw file bytes, 20,000 recursive `proc_macro2` token trees, and 4,096 bytes of content on any physical line. The token count charges one per leaf and one per group plus its recursive contents; whitespace and comments do not count. Line content excludes LF but retains a preceding CR. Whole-module `#[rustfmt::skip]`, including one applied by `cfg_attr`, is forbidden on both external and recursively nested inline modules; skips on individual non-module items remain permitted. Generated files are exempt from the line-count ratchet only with a generator path and hash, not from the absolute source-shape limits. |
