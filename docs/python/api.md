...
class CadAuthoredGraph:
    ...
    def planar_circular_section(self, *, classification_tolerance: float, region: str, x_lower: str, x_upper: str, y_lower: str, y_upper: str, hole: str) -> RectangleWithCircularHole: ...
    @property
    def canonical_bytes(self) -> bytes: ...
    @property
    def graph_digest(self) -> str: ...
...
### `eqiora.geometry.RectangleWithCircularHole`

```python
@final
class RectangleWithCircularHole:
    def __new__(cls, *, bounds: tuple[tuple[float, float], tuple[float, float]], circle_center: tuple[float, float], circle_radius: float, tolerance: float, region: str, x_lower: str, x_upper: str, y_lower: str, y_upper: str, hole: str) -> Self: ...
    @property
...
### `eqiora.meshing.CircularHoleChordalMesh`
...
### `eqiora.meshing.circular_hole_chordal`

```python
def circular_hole_chordal(geometry: RectangleWithCircularHole, *, max_boundary_error: float=..., required_minimum_mean_ratio: float=..., max_segments: int=...) -> CircularHoleChordalMesh: ...
```
...
These immutable types project only trajectories that have passed an exact
native replay. The current admitted constructor is fixed-step V1 over one
affine-triangle 2D mesh; the general names do not imply wider admission.
