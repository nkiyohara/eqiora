...
|---|---|---|
| Schäfer–Turek DFG 2D-2 and 2D-3, backward-facing step, and other non-box flow benchmarks | The exact DFG-shaped geometry and its source-bound 50-chord reference mesh now execute one coarse steady-Stokes witness. That is not the transient benchmark: canonical transient Navier–Stokes lowering remains Cartesian-box-specific, no general geometry-boundary embedding or production mesh family exists, and drag, lift, pressure-difference, refinement, and convergence evidence are absent. Backward-facing step needs the same general transient boundary and meshing path but not the circle primitive | `case:fluid.exact-circular-hole-stokes-2d`, `symbol:lower_transient_incompressible_navier_stokes_cartesian_2d` |
