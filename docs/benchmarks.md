...
|---|---|---|
| Schäfer–Turek DFG 2D-2 and 2D-3, backward-facing step, and other non-box flow benchmarks | Exact rectangle-minus-circle Geometry and bounded Gmsh meshing are available through the common product path. Transient Navier–Stokes remains rectangle-only at root admission; no public non-box transient Plan, production mesh family, drag, lift, pressure-difference, refinement, or convergence evidence exists yet. Backward-facing step needs the same general transient boundary and meshing path but not the circle primitive | `symbol:lower_transient_incompressible_navier_stokes_cartesian_2d` |
