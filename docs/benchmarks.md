...
|---|---|---|
| Schäfer–Turek DFG 2D-2 and 2D-3, backward-facing step, and other non-box flow benchmarks | The DFG channel and exact circular obstacle can now be retained as one canonical geometry artifact and admitted as Model spatial support. That does **not** yet make the benchmark executable: no source-bound chordal/curved Realization or general geometry-to-mesh correspondence supplies its facets, geometry boundaries still lack the embedding contract required by physical Ports, and canonical transient Navier–Stokes lowering remains Cartesian-box-specific. The existing non-zero inlet and constant-traction outlet wiring is reusable only after those seams close. Backward-facing step needs the same general-boundary lowering/correspondence path but not the circle primitive | `case:geometry.exact-circular-hole-geometry`, `symbol:lower_transient_incompressible_navier_stokes_cartesian_2d` |
