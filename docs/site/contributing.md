# Contributing

Eqiora welcomes focused contributions that preserve its meaning-first
architecture and evidence discipline.

## Before opening a change

1. Read the
   [contribution guide](https://github.com/nkiyohara/eqiora/blob/main/CONTRIBUTING.md).
2. Check the
   [architecture](https://github.com/nkiyohara/eqiora/blob/main/docs/architecture.md)
   and [glossary](https://github.com/nkiyohara/eqiora/blob/main/docs/glossary.md).
3. Search existing issues and RFCs before introducing a public concept.
4. Keep one vertical slice bounded enough to verify and review.

New public semantics, wire formats, stable interfaces, dependency-layer
exceptions, numerical contracts, or governance changes require the
[RFC process](https://github.com/nkiyohara/eqiora/blob/main/rfcs/README.md).

## Community and trust

- [Governance](https://github.com/nkiyohara/eqiora/blob/main/GOVERNANCE.md)
- [Code of Conduct](https://github.com/nkiyohara/eqiora/blob/main/CODE_OF_CONDUCT.md)
- [Security policy](https://github.com/nkiyohara/eqiora/blob/main/SECURITY.md)
- [Apache-2.0 license](https://github.com/nkiyohara/eqiora/blob/main/LICENSE)

Do not report a vulnerability in a public issue; follow the private process in
the security policy.
