...
Equation-structure operators such as `grad`, `div`, `trace`, and `coordinate`
belong to Eqiora's small implicit prelude. Scalar mathematical functions and
constants belong to the compiler-owned `math` namespace instead, currently
`math.sin` and `math.pi`. Neither group is imported, and user declarations or
packages cannot shadow `math`.
