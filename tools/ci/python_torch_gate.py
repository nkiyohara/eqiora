#!/usr/bin/env python3
"""Build an installed wheel environment and verify the PyTorch adapter."""
...
PACKAGE = ROOT
TEST = ROOT / "bindings/python/tests/test_torch.py"
...
TORCH = _DISTRIBUTION["tested-torch"]
...
def run(argv: list[str], *, cwd: Path = ROOT) -> None:
    ...
    environment.update(
        {
            "EQIORA_TEST_TORCH_VERSION": TORCH.removeprefix("torch=="),
...
def main() -> int:
    try:
        if uv := shutil.which("uv"):
            ...
            return 0
        with tempfile.TemporaryDirectory(prefix="eqiora-torch-gate-") as directory:
            environment = Path(directory)
            run([sys.executable, "-m", "venv", str(environment)])
            python = str(venv_python(environment))
            run([python, "-m", "pip", "install", MATURIN, PYTEST, TORCH])
            run(
                [
                    python,
                    "-m",
        ...
        print(f"Python PyTorch gate failed: {error}", file=sys.stderr)
        return 2
    return 0
