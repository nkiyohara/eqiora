...
def evaluate(relevance: Mapping[str, bool], results: Mapping[str, str]) -> list[str]:
    ...
    for job, surface in JOB_SURFACES.items():
        ...
        if relevance.get(surface) and result != "success":
            failures.append(f"required surface {surface} did not succeed in job {job}")
    return failures
...
def parse_relevance(raw: Mapping[str, object]) -> dict[str, bool]:
    ...
    expected = set(JOB_SURFACES.values())
    actual = set(raw)
    if actual != expected:
...
def main() -> int:
    try:
        ...
        results = {key: str(value) for key, value in raw_results.items()}
