...
library. It never imports Eqiora, NumPy, PyTorch, JAX, or Matplotlib, so
generating the reference cannot initialize the native module or an optional
framework.
"""
...
EXPECTED_COUNTS = {
    "modules": 11,
    "qualified_exports": 127,
    "grouped_declarations": 108,
    "classes": 78,
    "top_level_function_nodes": 39,
    "grouped_functions": 29,
    "aliases": 1,
    "method_nodes": 617,
    "non_dunder_method_nodes": 487,
    "dunder_method_nodes": 130,
    "visible_class_assignments": 47,
    "authoritative_member_summaries": 0,
    "signature_only_non_dunder_members": 487,
    "canonical_reexports": 11,
    "module_exports": 7,
    "version_exports": 1,
}
...
def parse_documentation(text: str | None, owner: str) -> Documentation:
    """Require the accepted summary-plus-authority form."""
    if not text:
    ...
    summary = normalized_summary(match.group(1).strip())
    authority = match.group(2)
    validate_repository_path(authority_path(authority), owner)
...
def validate_repository_path(value: str, owner: str) -> None:
    """Reject paths that the exact-SHA source component must reject."""
    path = Path(value)
    if (
        not value
        or path.is_absolute()
...
def literal_all(tree: ast.Module, owner: str) -> tuple[str, ...]:
    """Read exactly one literal string-only ``__all__`` assignment."""
...
def alias_documentation(
    ...
    """Read the contiguous ``#:`` block immediately above a public alias."""
...
def declaration_map(
    ...
    """Group overloads while preserving the accepted source identity."""
    declarations: dict[str, Declaration] = {}
    for node in tree.body:
...
def import_map(tree: ast.Module) -> dict[str, ImportBinding]:
    """Map relative imports without importing their targets."""
    imports: dict[str, ImportBinding] = {}
...
def discover_modules() -> tuple[ModuleData, ...]:
    """Discover the exact eligible stub set and parse it without imports."""
...
def classify_exports(modules: tuple[ModuleData, ...]) -> tuple[Export, ...]:
    """Resolve every public spelling to a direct, module, version, or re-export target."""
...
def validate_inventory(
    ...
    """Fail on any drift from the independently sealed inventory counts."""
    declarations = [
        declaration
        for module in modules
...
def decorator_lines(decorators: list[ast.expr], *, indent: str = "") -> list[str]:
    """Render decorators in source order."""
...
def function_signature(
    ...
    """Render an exact AST-derived function signature without a body."""
...
def assignment_signature(
    ...
    """Render a typed/value assignment without evaluation."""
    if isinstance(node, ast.AnnAssign):
...
def declaration_signature(declaration: Declaration) -> list[str]:
    """Render one canonical grouped declaration."""
...
def mdx_source_link(authority: str) -> str:
    """Render a static exact-build-SHA source link without tracking a SHA."""
...
def markdown_source_link(authority: str) -> str:
    """Render the repository Markdown's relative source link."""
    path = authority_path(authority)
    return f"[`{authority}`](../../{path})"


def mdx_frontmatter(title: str, description: str) -> list[str]:
    """Render deterministic YAML frontmatter and the exact-SHA component import."""
    return [
        "---",
...
def declaration_lookup(modules: tuple[ModuleData, ...]) -> dict[str, Declaration]:
    """Build a unique canonical declaration lookup."""
    return {
        declaration.qualified: declaration
        for module in modules
        for name, declaration in module.declarations.items()
...
def render_mdx_export(
    ...
    """Render one public spelling exactly once on its owning module page."""
...
def render_mdx_module(
    ...
    """Render one Starlight module page."""
...
def render_mdx_index(modules: tuple[ModuleData, ...], counts: dict[str, int]) -> str:
    """Render the public Python landing with the honest documentation boundary."""
    lines = mdx_frontmatter(
        "Python API",
    ...
    lines.extend(
        [
            ...
            "| --- | ---: | --- |",
            f"| Public modules | {counts['modules']} | 11/11 reviewed module summaries |",
            ...
            f"| Canonical grouped declarations | {counts['grouped_declarations']} | 108/108 reviewed declaration summaries |",
...
def render_markdown_export(
    ...
    """Render one spelling in the repository-native all-in-one reference."""
    lines = [
...
def render_markdown(
    ...
    """Render the repository-native complete all-in-one reference."""
...
def render_outputs() -> dict[Path, str]:
    """Validate the accepted inventory and return every deterministic output."""
...
def check(outputs: dict[Path, str]) -> int:
    """Compare all committed projections without writing any path."""
    failures = 0
...
def write(outputs: dict[Path, str]) -> int:
    """Update only the contract-listed generated outputs."""
    for path, rendered in outputs.items():
