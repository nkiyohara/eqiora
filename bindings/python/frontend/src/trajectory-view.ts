...
export function render({ model, el }: Context): () => void {
	...
	canvas.width = 960;
	canvas.height = 480;
	const canvasDescription = `Eqiora Trajectory ${trajectory.trajectoryDigest}; field ${trajectory.fieldId}; coherent-SI dimension [${trajectory.dimension.join(", ")}]; ${trajectory.frame} frame; ${trajectory.steps.length} stored states.`;
	canvas.setAttribute("role", "img");
	canvas.setAttribute("aria-label", canvasDescription);
	canvas.textContent = canvasDescription;
