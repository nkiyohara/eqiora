...
> **Alpha — `0.1.0a7`.** The supported boundary is intentionally narrow.
> Consult the [capability matrix](https://eqiora.org/capabilities/) before
> relying on a method, backend, or platform.

## Install

Eqiora `0.1.0a7` supports ordinary-GIL CPython 3.11–3.14 on
manylinux x86-64:
...
model = eqiora.compile(
    ...
    public component DifferentiatedPoisson() {
      ...
      relation y_lower_value on y_lower {
        trace(potential) - boundary_offset = 0;
      }
      relation y_upper_value on y_upper {
        trace(potential) - boundary_offset = 0;
      }
    }
    """,
...
`0.1.0a7` is an alpha prerelease. Public Python names and serialized contracts
