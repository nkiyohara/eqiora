...
The N1 acceptance predicate retains its pinned notebook-host and bounded process
cleanup checks on CPython 3.13. The displaced exact-Mesh rich viewer and its
fixed Gmsh output contract are no longer part of candidate admission; the
provider-neutral trajectory presentation remains separately covered.
...
profiles, the closed frontend asset/license/browser identity, and a detached
candidate-bound H2 receipt. Its H2 predicate requires two clean locked frontend
builds to have the same complete output paths, modes, sizes, and bytes. That is
...
four wheels, the manifest, and the requested profile. A native display-hook
token, frontend or `_presentation` path, semantic anywidget dependency,
...
- any N1 signal hidden in a v2 family, including a hook-only, asset-only,
  frontend-path-only, one-wheel-only, or sdist-only signal;
- missing, mixed, duplicate, ranged, unmarked, differently marked, or
  interpreter-specific anywidget metadata in any wheel;
- a v3 manifest with a missing, extra, misspelled, or incorrectly typed
  frontend, asset, license, runtime, browser, or Notebook-check member;
...
- a non-registry or integrity-free lock node, changed lifecycle-script
  inventory, source map, external import/request, unequal rebuild, unmapped
  emitted module, asset/notice drift, or wrong exact browser identity;
