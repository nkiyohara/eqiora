...
vertex and cell indices are last-axis-fastest, and each hypercube connectivity
row uses tensor-product/Z local order. Correspondence retains the one body and
the exact four boundary facet inventories. The snapshot retains the
solver-owned binary64 displacement projection without recomputing an analytic
field. Scalar and other structurally valid physical tuples remain valid
low-level Q1 snapshot grammar; exact Field replay rejects them for this vector
