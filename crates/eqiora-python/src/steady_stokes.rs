...
use eqiora::artifact::{ModelDecoderLimits, ModelEnvelopeV7};
...
///
/// The private generic keeps coordinate and connectivity lifetime rules
/// identical without adding a public multidimensional Array contract.
struct ReadOnlyMatrix<T: Element> {
    rows: usize,
    columns: usize,
...
impl<T: Element> ReadOnlyMatrix<T> {
    fn new(rows: usize, columns: usize, values: Vec<T>) -> Self {
    ...
    fn numpy(&self, py: Python<'_>) -> PyResult<Py<PyArray2<T>>> {
        let values = {
...
#[pyfunction]
#[pyo3(signature = (*, model_v7, geometry, mesh))]
pub(crate) fn solve_exact_cylinder_stokes(
    ...
    panic_boundary(py, || {
        ...
        let native = py.detach(move || {
            ...
            let model = ModelEnvelopeV7::from_json(&model_v7, ModelDecoderLimits::default())?;
