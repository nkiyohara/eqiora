...
fn typed_literal_lowering_preserves_type_through_detachment_and_zero_negation() {
    ...
    let scalar = ValueType::scalar(ScalarDomain::Complex, DimExponents::DIMENSIONLESS).expect("admitted numeric scalar type");
...
fn typed_cartesian_coordinates_require_real_scalar_lengths() {
    ...
    for value_type in [
        ...
        ValueType::scalar(ScalarDomain::Real, length).expect("admitted numeric scalar type")
            .array(1)
            .unwrap(),
    ] {
        let syntax = eqiora_lang::ValueTypeSyntax::from_checked(&value_type, |_| None).unwrap();
...
"#;
    ...
    let electrical = eqiora_lang::DraftPhysicalDomain::new(
        "electrical",
        eqiora_core::ValueType::scalar(eqiora_core::ScalarDomain::Real, voltage_dimension()).expect("admitted numeric scalar type"),
        eqiora_core::ValueType::scalar(eqiora_core::ScalarDomain::Real, current_dimension()).expect("admitted numeric scalar type"),
    );
    ...
    let resistance = eqiora_lang::DraftParameter::new(
        "resistance",
        eqiora_core::ValueLiteral::from_real(
            eqiora_core::ValueType::scalar(eqiora_core::ScalarDomain::Real, resistance_dimension()).expect("admitted numeric scalar type"),
            2.0,
        )
        .unwrap(),
    );
