...
/// Only literal constructor leaves inherit declaration units; arithmetic never does.
pub(crate) fn initializer(expression: &eqiora_lang::Expr, target: &ValueType) -> eqiora_lang::Expr {
    use eqiora_lang::{ExprKind, SourceAstFactory, UnaryOp};
    let kind = match expression.kind() {
        ExprKind::Array(elements) if target.array_rank() > 0 => {
            let element = eqiora_schema::kernel::typing::index(
                eqiora_schema::kernel::typing::ExpressionType::<()>::new(target.clone(), None),
                0,
            )
            .expect("checked nonempty array")
            .value_type;
            ExprKind::Array(
                elements
                    .iter()
                    .map(|value| initializer_leaf(value, &element))
                    .collect(),
            )
        }
        ExprKind::Call { callee, arguments }
            if callee.as_str() == "math.complex" && target.shape().is_scalar() =>
        {
            let scalar = ValueType::scalar(eqiora_core::ScalarDomain::Real, target.dimension());
            ExprKind::Call {
                callee: callee.clone(),
                arguments: arguments
                    .iter()
                    .map(|value| initializer_leaf(value, &scalar))
                    .collect(),
            }
        }
        ExprKind::Unary {
            ...
            op: UnaryOp::Neg,
            value: Box::new(initializer(value, target)),
        },
        _ => return expression.clone(),
    };
    SourceAstFactory::expression(kind, expression.range()).expect("retained initializer structure")
}

fn initializer_leaf(expression: &eqiora_lang::Expr, target: &ValueType) -> eqiora_lang::Expr {
    use eqiora_lang::{ExprKind, SourceAstFactory, UnaryOp};
    if target.shape().is_scalar() {
        ...
        if let Some(value) = value {
            return SourceAstFactory::expression(
                ExprKind::Quantity {
                    value,
            ...
            .expect("typed numeric initializer leaf");
        }
    }
    initializer(expression, target)
}
