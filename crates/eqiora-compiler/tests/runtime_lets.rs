...
fn runtime_scalar_aliases_and_identity_evolution_work_in_both_containers() {
    ...
    accepted(
        "component C(state x: 1) { let y = z * x; let z = x + 1; let identity = x; relation r { derivative(identity) = y / 1[s]; } } model M { state x: 1; instance c: C(field x = x); }",
    );
    rejected(
...
fn runtime_effects_do_not_enter_static_bindings_transitively() {
    rejected(
        "component C() { public parameter p: 1; } model M { state x: 1; let a = x; let b = a+1; instance c: C(p = b); }",
    );
    rejected(
        "component C(state x: 1) { let a=x; let b=a+1; instance d: D(p=b); } component D() { public parameter p: 1; } model M {}",
    );
...
fn parameter_only_scalar_math_aliases_remain_static_and_keep_domain_checks() {
    ...
    accepted(
        "component C() { public parameter p: 1; relation r { p=0; } } model M { parameter p: 1=4; let a=math.sqrt(p); let b=math.sin(a); instance c:C(p=b); }",
    );
    accepted(
        "component C() { public parameter p: 1; let a=math.sqrt(p); instance d:D(p=a); } component D() { public parameter p:1; relation r { p=0; } } model M { parameter p:1=4; instance c:C(p=p); }",
    );
    rejected("model M { parameter p:1=-1; let a=math.sqrt(p); relation r { a=0; } }");
    rejected(
        "component C() { public parameter p:1; let a=math.sqrt(p); relation r { a=0; } } model M { instance c:C(p=-1); }",
    );
}
