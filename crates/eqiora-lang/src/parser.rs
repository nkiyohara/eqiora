...
impl Parser<'_> {
    ...
    fn parse_domain(&mut self) -> Option<DomainDecl> {
        ...
        let syntax = if self.at_keyword("box") {
            ...
            let mut coordinates = vec![self.parse_signed_number()?];
            while self.at(TokenKind::Comma) {
                self.bump();
                coordinates.push(self.parse_signed_number()?);
            }
    ...
    fn parse_instance(&mut self) -> Option<InstanceDecl> {
