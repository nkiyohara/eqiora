...
mod tests {
    ...
    fn f64_zero_normalizes_and_negative_zero_wire_fails() {
        ...
        assert!(
            ResolvedArrayV1::from_json(&negative, DecoderLimits::default())
                .unwrap_err()
                .message()
    ...
    fn tag_shape_unknown_data_and_limits_fail_closed() {
        for invalid in [
            ...
            assert!(ResolvedArrayV1::from_json(invalid, DecoderLimits::default()).is_err());
        }
