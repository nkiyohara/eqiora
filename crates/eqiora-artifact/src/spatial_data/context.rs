...
/// Private proof for the one standalone prescribed-solid artifact family.
///
/// The public family methods construct this token internally. It never turns
/// raw Field or Domain identities into roles: those are first rederived by the
/// closed Realization against the exact current Model.
#[derive(Debug)]
...
impl<'a> ValidatedPrescribedDynamicSolidContext<'a> {
    pub(super) fn new(
        ...
        let active_cells = correspondence
            ...
            .ok_or_else(|| {
                invalid_artifact("prescribed dynamic-solid body has no exact mesh cells")
            })?;
        if active_cells
            .iter()
            .copied()
        ...
        {
            return Err(invalid_artifact(
                "prescribed dynamic-solid body does not cover the complete exact mesh",
            ));
        }
        Ok(Self {
            replayed_model,
    ...
    pub(super) fn active_cells(&self, domain: Id<kinds::Domain>) -> Result<Vec<usize>, Diagnostic> {
        if domain != self.realization.solid_domain() {
            return Err(invalid_artifact(
                "prescribed dynamic-solid Field support differs from its exact body",
            ));
        }
        Ok(self.active_cells.clone())
    }
...
impl ValidatedFieldSnapshotContext for ValidatedPrescribedDynamicSolidContext<'_> {
    ...
    fn realized_field_space(
        ...
        if field != realization.displacement_field() && field != realization.velocity_field() {
            return Err(invalid_artifact(
                "Field snapshot Field is absent from the prescribed-solid Realization",
            ));
        }
        Ok((
            realization.solid_domain(),
