...
pub(super) fn point_inside_cartesian(point: &[f64], bounds: &[(f64, f64)], tolerance: f64) -> bool {
...
pub(super) fn cartesian_facet_roles(
    points: &[Vec<f64>],
    bounds: &[(f64, f64)],
    tolerance: f64,
...
fn reject_ambiguous_entity_sets(region: &PlanarRegion) -> Result<(), Diagnostic> {
...
fn region_edges(region: &PlanarRegion) -> Vec<RegionEdge> {
...
mod adjacent_partition_tests {
    ...
    fn exact_partition_replays_mesh_correspondence_and_parent_relative_interface() {
        ...
        production
            ...
            .unwrap();
        assert!(GeometryMeshCorrespondenceEnvelopeV1::from_adjacent_rectangle_partition_affine_triangles(&geometry, [1, 2]).is_err());
