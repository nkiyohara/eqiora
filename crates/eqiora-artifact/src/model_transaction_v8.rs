//! Explicit transaction wire v8 for direct Cartesian coordinate sources.
...
/// Explicit v8 serialization of one ordered Semantic Model transaction.
///
/// V8 inherits the complete v7 edit grammar and adds only Cartesian
/// coordinate-source Domain definitions plus their Domain-to-Parameter
/// dependencies.
#[derive(Debug, Clone, PartialEq)]
pub struct ModelTransactionEnvelopeV8 {
    inner: ModelTransactionEnvelopeV2,
}

impl ModelTransactionEnvelopeV8 {
    ...
    pub fn from_transaction(transaction: &Transaction) -> Result<Self, Diagnostic> {
        ModelTransactionEnvelopeV2::from_transaction_v8(transaction).map(|inner| Self { inner })
    }

    /// Decode and validate the local v8 edit grammar without graph mutation.
    ///
    /// # Errors
    ...
    pub fn from_json(bytes: &[u8], limits: ModelDecoderLimits) -> Result<Self, Diagnostic> {
        ModelTransactionEnvelopeV2::from_json_v8(bytes, limits).map(|inner| Self { inner })
    }
    ...
    /// Domain-separated SHA-256 identity of the exact ordered v8 transaction.
    ///
    /// # Errors
    /// Returns `EQ0901` if serialization unexpectedly fails.
