...
fn fieldwise_v2_round_trips_minres_and_exact_mixed_identity() {
    ...
    assert!(text.contains("eqiora.realization-envelope/v2"));
...
#[test]
fn realization_v2_golden_bytes_are_frozen() {
...
fn fieldwise_v2_scale_changes_are_identity_and_invalid_scales_fail_closed() {
    ...
    wrong_coordinate_dimension["plan"]["spatial"]["coordinate_length_scale"]["dimension"]["length"] =
        serde_json::json!(0);
    assert!(
        RealizationEnvelopeV2::from_json(
...
#[test]
fn fieldwise_v2_rejects_field_constraint_and_block_ambiguity() {
    let fixture = fixture(ReductionPolicy::Reproducible);
    let bytes = fixture.realization.canonical_json().unwrap();
...
#[test]
fn fieldwise_v2_decoder_limits_each_identity_inventory() {
    let fixture = fixture(ReductionPolicy::Reproducible);
    let bytes = fixture.realization.canonical_json().unwrap();
...
#[test]
fn run_v2_accepts_fieldwise_v2_and_rejects_reduction_or_topology_drift() {
    let fixture = fixture(ReductionPolicy::Reproducible);
...
const fn dimension(mass: i8, length: i8, time: i8) -> DimExponents {
    DimExponents {
        mass,
        length,
        time,
