...
fn accepted_execution_links_the_same_document_and_schema_facts() {
    ...
    assert_eq!(model.schema(), "eqiora.model-envelope/v19");
    assert_eq!(
        model.transaction_schema(),
        "eqiora.model-transaction-envelope/v19"
    );
...
fn response_model_identity_uses_the_frozen_character_bound() {
    let value = serde_json::json!({
        ...
        "outcome": {
            "status": "accepted",
            "model": {
                "schema": "eqiora.model-envelope/v19",
                "transactionSchema": "eqiora.model-transaction-envelope/v19",
                "digest": "0".repeat(64),
