...
use super::{PrescribedDynamicSolidStateRun3d, invalid};
...
pub struct PrescribedDynamicSolidExternalProviderStateRun3d {
    direct: PrescribedDynamicSolidStateRun3d,
    provider_occurrence: PrescribedDynamicSolidProviderOccurrenceEnvelopeV1,
    run: RunManifestV2,
...
impl PrescribedDynamicSolidExternalProviderStateRun3d {
    ...
    pub fn solve_reference_with_connected_subprocess(
        ...
        let hello = expect_hello(session.receive_control(cancellation)?)?;
        hello.validate()?;
        ...
        session.send_control(&exchange.bind, cancellation)?;
        let bound = expect_bound(session.receive_control(cancellation)?)?;
        exchange.validate_bound(&bound)?;
        session.send_control(&exchange.evaluate, cancellation)?;
        session.send_bulk(exchange.displacement_bulk.clone(), cancellation)?;
        session.send_bulk(exchange.velocity_bulk.clone(), cancellation)?;
        let candidate_control = expect_candidate(session.receive_control(cancellation)?)?;
        exchange.validate_candidate_control(&candidate_control)?;
        let candidate_bulk = session.receive_bulk(cancellation)?;
        let (candidate_identity, candidate) =
            exchange.admit_candidate(&candidate_control, &candidate_bulk)?;
        let report = expect_report(session.receive_control(cancellation)?)?;
        ...
        session.send_control(&close, cancellation)?;
        let closed = expect_closed(session.receive_control(cancellation)?)?;
    ...
    pub fn revalidate(&self) -> Result<(), Diagnostic> {
        ...
        if !composition::candidate_matches_exact(&self.evidence.candidate)
            || protocol::decode_candidate(&self.evidence.candidate_bulk)? != self.evidence.candidate
        ...
        self.evidence.transcript.validate_success()?;
    ...
    fn revalidate_with_exchange(&self, exchange: &Exchange) -> Result<(), Diagnostic> {
        self.revalidate()?;
        if exchange.candidate_identity_for_bulk(&self.evidence.candidate_bulk)
...
fn provider_error<T>(error: ErrorControl, expected_phase: &str) -> Result<T, Diagnostic> {
    ...
    Err(invalid(format!(
        "provider rejected the {expected_phase} phase with a bounded error control"
    )))
}
