...
| `transient-cylinder-wake` | [`python/transient_cylinder_wake.py`](python/transient_cylinder_wake.py) | From an installed package, compose the Python-authored cylinder Geometry, packaged steady and transient equation Components, common Mesh and typed policies, one accepted nonzero transient State, typed cell-average vorticity, and an optional caller-owned Figure. This bounded product example is explicitly unverified. |
| `transient-cylinder-wake-marimo` | [`python/transient_cylinder_wake_marimo.py`](python/transient_cylinder_wake_marimo.py) | In Marimo, follow the same public transient workflow, inspect exact Geometry/Mesh/Model/Plan/Trajectory lineage, and render the typed cell-average vorticity through a caller-owned Figure. The experience is explicitly unverified. |
| `transient-cylinder-wake-jupyter` | [`python/transient_cylinder_wake_jupyter.ipynb`](python/transient_cylinder_wake_jupyter.ipynb) | In Jupyter, mirror the Marimo composition and expose the same accepted State, typed vorticity, lineage summary, and caller-owned Figure without a Mesh display hook. The experience is explicitly unverified. |
