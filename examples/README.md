...
| `exact-cylinder-stokes-jupyter` | [`python/exact_cylinder_stokes_jupyter.ipynb`](python/exact_cylinder_stokes_jupyter.ipynb) | In JupyterLab, run the same installed-package composition and inspect the same carrier identities, typed evidence fields, and caller-owned pressure Figure without stored outputs or a forked scientific workflow. |
