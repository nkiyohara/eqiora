...
| `project-modules` | [`modules/resistor-project`](modules/resistor-project/) | Compile a closed, portable multi-file source inventory whose `.eqi` module declarations own logical identity independently of file paths; the optional `package-filesystem` run discovers that same closure through bounded no-follow directory traversal. |
