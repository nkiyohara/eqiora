...
| `transient-cylinder-wake-jupyter` | [`python/transient_cylinder_wake_jupyter.ipynb`](python/transient_cylinder_wake_jupyter.ipynb) | In Jupyter, mirror the Marimo composition and expose the same final accepted startup State, typed vorticity, lineage summary, and caller-owned Figure without a Mesh display hook. The experience is explicitly unverified. |
